...
func (s *Service) CreateSample(input model.Sample) (*model.Sample, error) {
	return s.CreateSampleContext(context.Background(), input)
}

func (s *Service) CreateSampleContext(ctx context.Context, input model.Sample) (*model.Sample, error) {
	...
	active := s.sampleContexts.take(ctx)
	if err := s.store.CreateSampleContext(active, sa); err != nil {
		return sa, err
	}
	return sa, nil
}
